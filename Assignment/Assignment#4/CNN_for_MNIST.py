...
class Conv2d(Layer):
    ...
    def compute_output_dims(h_in, w_in, h_f, w_f, h_p, w_p, stride):
        ...
        h_out = int(np.ceil(h_in - h_f + h_p * 2) / stride) + 1
        w_out = int(np.ceil(w_in - w_f + w_p * 2) / stride) + 1
    ...
    def backward(self, da):
        ...
        # print('w shape: '+str(self.w.shape)+' da shape: '+str(da.shape)+' x_pad shape: '+str(self.x_pad.shape))
        for i in range(h_out):
            h_start = i * self.stride
            h_end = h_start + h_f
            for j in range(w_out):
                ...
                # complete your code here
                # print('shape: '+ str(self.w[np.newaxis, :, :, :,].shape)+'shape: '+str(da[:,:, np.newaxis,i:i+1,j:j+1].shape))
                out[:, :, h_start:h_end, w_start:w_end] += np.sum(self.w[np.newaxis, :, :,:,] * da[:, :, np.newaxis, i:i+1, j:j+1], axis=1)
...
class MaxPool2d(Layer):
    ...
    def compute_output_dims(h_in, w_in, h_f, w_f, stride):
        ...
        """
        # complete your code here
        h_out = int(np.ceil(h_in - h_f) / stride) + 1
        w_out = int(np.ceil(w_in - w_f) / stride) + 1
        return h_out, w_out
...
class FullyConnected(Layer):
    ...
    def backward(self, da):
        ...
        """
        # complete your code here
        self.dw = np.dot(da.T, self.x) / da.shape[0]
        self.db = np.sum(da, axis=0) / da.shape[0]
...
def main():
    ...
    model = SequentialModel([
        ...
        # complete your code here
        FullyConnected(h*w*3//(2*2), num_classes),
        Softmax()
    ])
